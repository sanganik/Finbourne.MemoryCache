...
namespace Finbourne.Cache.Component
{
    public class FinbourneCacheService : IFinbourneCacheService
    {
        ...
        {
            if (_memoryCache.Get().TryGetValue(key, out T type))
                return type;
        ...
        public async Task Set<T>(string key, T type)
        {
            ...
            if (_memoryCache.Get().TryGetValue(key, out T cachedType))
                return;
            ...
            try
            {
                ...
                if (_memoryCache.Get().TryGetValue(key, out cachedType))
                    return;
                ...
                var stats = _memoryCache.Get().GetCurrentStatistics();

                // better to work with percentile rather than strict limit, but for the purpose of the test
                if(stats.CurrentEntryCount >= _memoryCache.SizeLimit)
                {
                    ForceEvictLRUCachedItem();
                }

                _memoryCache.Get().Set(key, type, cacheEntryOptions);
        ...
        private void ForceEvictLRUCachedItem()
        {
            ...
            var percentileToRemove = (1 / _memoryCache.SizeLimit * 100);
            _memoryCache.Get().Compact(Convert.ToDouble(percentileToRemove));
        }
